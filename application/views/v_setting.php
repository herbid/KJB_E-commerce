<div class="col-md-12">
	...
	<div class="card card-primary">
		...
		echo form_open('admin/set_lokasi');
		?>	
			...
			<div class="row">
				<div class="col-sm-6">
					<div class="form-group">
						...
						<input type="text" name="nohp" class="form-control" value="<?=$setting->nohp ?>" required>
					</div>
				</div>
		...
		<div class="card-footer">
			...
			<a href="<?=base_url('admin')?>" class="btn  btn-sm btn-outline-warning  "> kembali</a>
		</div>
		<?php echo form_close() ?>
	...
	<script>
		$(document).ready(function () {
			$.ajax({
				type: "POST",
				url: "<?=base_url('rajaongkir/provinsi_a')?>",
			...
			$("select[name=provinsi]").on("change", function () {
				...
				$.ajax({
					type: "POST",
					url: "<?=base_url('rajaongkir/kota_a') ?>",
